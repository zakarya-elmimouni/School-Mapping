...
DATA_YAML = 'dataset/peru/manual_labeled_data/data.yaml'  # path to you data.yaml file 
...
class YOLOObjective:
    def __init__(self, log_path="results/peru/ecp_yolo10n_finetuning_params_log.csv"): # change if needed
    ...
    def __call__(self, x):
        ...
        save_dir="results/peru/rslt_yolo10n_finetuning_auto_on_golden_best_params" # change if needed

        try:
            model = YOLO("results/peru/rslt_yolo10n_auto_labeling/exp/weights/best.pt") # # existing model pretrained on auto-labeled data (change if needed)
            ...
            results = model.train(
                ...
                save_period=20,
                verbose=False,
                project="results/peru/rslt_yolo10n_finetuning_auto_on_golden_best_params", # change if needed (this is where yo want to store results
                name=exp_name,
            )
